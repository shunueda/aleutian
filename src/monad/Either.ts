...
export abstract class Either<out A, out B>
  ...
  public override toString(): string {
    return `${Either.name}.` + this.fold(
      left => `Left(${addDoubleQuoteIfString(left)})`,
      right => `Right(${addDoubleQuoteIfString(right)})`
    )
  }
}
